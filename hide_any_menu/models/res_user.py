...
class FieldConfiguration(models.Model):
    ...
    model_id = fields.Many2one('ir.model', string='Model', required=True, ondelete='set default')
    field_id = fields.Many2one('ir.model.fields', string='Field', required=True, ondelete='set default')
    field_name = fields.Char(related='field_id.name', string='Technical Name', readonly=True)
    group_ids = fields.Many2many('res.groups', 'field_config_group_rel', 'group_id', 'field_config_id', required=True, string='Groups', ondelete='cascade')
