odoo.define("account.ReconciliationRenderer", function (require) {
  ...
  var StatementRenderer = Widget.extend(FieldManagerMixin, {
    ...
    update: function (state) {
      ...
      if (
        state.valuenow === state.valuemax &&
        !this.$(".done_message").length
      ) {
  ...
  var LineRenderer = Widget.extend(FieldManagerMixin, {
    ...
    start: function () {
        ...
        self.fields = {
          partner_id: new relational_fields.FieldMany2One(
            ...
            self.model.get(recordID),
            {
              ...
              attrs: {
                placeholder:
                  self._initialState.st_line.communication_partner_name ||
                  _t("Select Partner"),
              },
            }
          ),
      ...
      console.log("on reconcile");
      console.log(this);

      if (
        this.model.context.is_uudp &&
        this.model.context.account_id &&
        !this.model.context.is_reimburse
      ) {
        var accountId = this.model.context.account_id;
        this._rpc({
          model: "account.account",
          method: "search_read",
          domain: [["id", "=", accountId]],
          fields: ["analytic_account_id", "id", "display_name"],
        }).then(function (account) {
          var analyticAccObj = account[0].analytic_account_id;
          var analyticAccountId = {
            id: analyticAccObj[0],
            display_name: analyticAccObj[1],
          };
          var account_id = {
            account_id: { id: account[0].id, name: account[0].name },
            analytic_account_id: analyticAccountId,
          };
          self.trigger_up("update_proposition", { data: account_id });
        });
      } else if (
        this.model.context.is_reimburse &&
        this.model.context.coa_debit_ids
      ) {

        if (this.model.context.is_multi) {
          console.log("multi");
          _.each(this.model.statement.lines, (lines) => {
            _.each(this.model.context.coa_debit_ids, (res) => {
              var account_id = {
                account_id: {
                  id: res.account_id.id,
                  name: res.account_id.name,
                  amount: res.amount,
                },
              };

              if (lines.st_line.uudp_id == res.uudp_id) {
                self.trigger_up("create_proposition", {
                //   data: account_id,
                  mode: "create",
                });
                self.trigger_up("update_proposition", { data: account_id });
          ...
          _.each(this.model.context.coa_debit_ids, (res) => {
            var account_id = {
              account_id: { id: res.account_id.id, name: res.account_id.name },
              amount: res.amount,
            };
            self.trigger_up("create_proposition", {
              data: account_id,
    ...
    update: function (state) {
      ...
      self
        .$("caption .o_buttons button.o_validate")
        .toggleClass("text-warning", to_check_checked);
      ...
      // Mv_lines
      var matching_modes = self.model.modes.filter((x) =>
        x.startsWith("match")
      );
      for (let i = 0; i < matching_modes.length; i++) {
        ...
        this.$(
          ...
          .toggleClass(
            "d-none",
            stateMvLines.length === 0 && !state["filter_" + matching_modes[i]]
          );
            ...
            .then(function () {
              ...
              _.each(self.fields, function (field, fieldName) {
                ...
                if (
                  ...
                  !_.isEqual(state.createForm[fieldName], data[fieldName])
                ) {
                  field.reset(record);
                }
                if (fieldName === "tax_ids") {
                  if (
                    ...
                    var price_include =
                      state.createForm[fieldName][0].price_include;
                    var force_tax_included =
                      state.createForm[fieldName][0].force_tax_included;
                    self
                      .$(".create_force_tax_included input")
                      .prop("checked", force_tax_included);
                    self
                      .$(".create_force_tax_included input")
                      .prop("disabled", price_include);
                  }
                }
              });
              if (state.to_check) {
      ...
      this.$(".create .add_line").toggle(
        Boolean(state.balance.amount_currency)
      );
    },
    ...
    _onFieldChanged: function (event) {
      ...
      if (fieldName === "partner_id") {
        ...
        var accountId = event.data.changes.account_id;
        var self = this;
        this._rpc({
          model: "account.account",
          method: "search_read",
          domain: [["id", "=", accountId.id]],
  ...
  var ManualLineRenderer = LineRenderer.extend({
    ...
    _renderCreate: function (state) {
      ...
      var parentPromise = this._super(state).then(function () {
        ...
        self
          .$(".create .create_journal_id .o_input")
          .addClass("o_required_modifier");
      });
