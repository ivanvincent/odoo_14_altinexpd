...
class PurchaseReport(models.TransientModel):
    ...
    def _quantity_to_str(self, from_uom, to_uom, qty):
        ...
        qty = from_uom._compute_quantity(
            qty, to_uom, rounding_method='HALF-UP')
    ...
    def get_lines(self, line_id=None, **kw):
        ...
        if report_type == 'vendor':
            line_ids = self.env['report.po.supplier'].browse(
                [model_id]).line_ids
            for line in line_ids:
                lines.append({
                    ...
                    'res_id': model_id,
                    'res_model': model,
                    'partner_id': {'id':line.partner_id.id,"name":line.partner_id.name,'res_model':line.partner_id._name},
                    'amount': line.amount,
                    'columns': [line.partner_id.name,
                                line.amount],
                    'level': level,
                })
    ...
    def _get_html(self):
        ...
        result['html'] = self.env.ref(
            'inherit_purchase_order.report_purchase_all_vendor')._render(rcontext)
        _logger.warning(rcontext)
        return result
