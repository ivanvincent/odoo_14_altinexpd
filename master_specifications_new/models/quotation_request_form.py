...
class QuotationRequestForm(models.Model):
    _name = 'quotation.request.form.new'
    ...
    line_ids = fields.One2many('quotation.request.form.line.new', 'qrf_id', 'Line')
    ...
    drawing_attachment_line_ids = fields.One2many('drawing.attachment.new', 'qrf_id', 'Drawing')
    qrf_attachment_line_ids = fields.One2many('qrf.attachment.new', 'qrf_id', 'QRF')
    ...
    po_attachment_line_ids = fields.One2many('po.attachment.new', 'qrf_id', 'QRF')
    ...
    sj_attachment_line_ids = fields.One2many('sj.attachment.new', 'qrf_id', 'SJ')
    ...
    def generate_report_file(self):
        if self.type == '1':
            report_template_id = self.env.ref('master_specifications_new.action_print_template_so_1_new')._render_qweb_pdf(self.id)
        elif self.type in ('2','4'):
            report_template_id = self.env.ref('master_specifications_new.action_print_template_so_2_new')._render_qweb_pdf(self.id)
        elif self.type == '3':
            report_template_id = self.env.ref('master_specifications_new.action_print_template_so_3_new')._render_qweb_pdf(self.id)
    ...
    def create(self, vals):            
        ...
        seq_id = self.env.ref('master_specifications_new.qrf_seq_new')
    ...
    def action_revise_qrf(self):
        return {
            ...
            'name'      : "Send",
            'res_model' : 'revise.wizard.new',
            'target'    : 'new',
            'view_id'   : self.env.ref('master_specifications_new.revise_wizard_form_new').id,
            'view_mode' : 'form',
    ...
    def action_cancel_po(self):
        return {
            ...
            'res_model' : 'revise.wizard.new',
            'target'    : 'new',
            'view_id'   : self.env.ref('master_specifications_new.cancel_wizard_form_new').id,
            'view_mode' : 'form',
            'context'   : {'default_qrf_id': self.id,},
        }
    ...
    def action_send_to_customer(self):
        ...
        return {
            'type'      : 'ir.actions.act_window',
            'name'      : "Send Mail",
            'res_model' : 'customer.mail.wizard.new',
            'target'    : 'new',
            'view_id'   : self.env.ref('master_specifications_new.send_customer_mail_form_new').id,
            'view_mode' : 'form',
            'context'   : {'default_qrf_id': self.id, 
    ...
    def action_send_production(self):
        ...
        # elif not mrp:
            ...
            return {
                'type'      : 'ir.actions.act_window',
                'name'      : "Detail",
                'res_model' : 'send.production.wizard.new',
                'target'    : 'new',
                'view_id'   : self.env.ref('master_specifications_new.send_production_wizard_form_new').id,
                'view_mode' : 'form',
...
#note tj
    def action_print(self):
        return {
            'type'      : 'ir.actions.act_window',
            'name'      : "Print",
            'res_model' : 'print.qrf.wizard.new',
            'target'    : 'new',
            'view_id'   : self.env.ref('master_specifications_new.print_qrf_wizard_form_new').id,
            'view_mode' : 'form',
            'context'   : {'default_qrf_id': self.id,},
        }
    ...
    def action_print_dqups2(self):
        return {
            ...
            'res_model' : 'print.qrf.wizard.new',
            'target'    : 'new',
            'view_id'   : self.env.ref('master_specifications_new.print_qrf_dqups2_form_new').id,
            'view_mode' : 'form',
            'context'   : {'default_qrf_id': self.id,},
        }
    ...
    def action_print_dqups3(self):
        return {
            ...
            'res_model' : 'print.qrf.wizard.new',
            'target'    : 'new',
            'view_id'   : self.env.ref('master_specifications_new.print_qrf_dqups3_form_new').id,
            'view_mode' : 'form',
            'context'   : {'default_qrf_id': self.id,},
        }
    ...
    def action_print_inform_consent(self):
        if not self.ic_doc_number:
            seq_id = self.env.ref('master_specifications_new.qrf_ic_no_doc_new')
            self.ic_doc_number = seq_id.next_by_id()

        return {
            'type'      : 'ir.actions.act_window',
            'name'      : "Print",
            'res_model' : 'inform.consent.wizard.new',
            'target'    : 'new',
            'view_id'   : self.env.ref('master_specifications_new.inform_consent_wizard_form_new').id,
            'view_mode' : 'form',
            'context'   : {'default_qrf_id': self.id,},
        }
...
class QuotationRequestFormLine(models.Model):
    _name = 'quotation.request.form.line.new'
    
    qrf_id = fields.Many2one('quotation.request.form.new', string='QRF')
    jenis_id = fields.Many2one('master.jenis.new', string='Jenis')
    jenis2_id = fields.Many2one('master.jenis.new', string='Jenis 2')
    line_spec_ids = fields.One2many('quotation.request.form.line.specification.new', 'qrf_line_id', 'Line Spec', ondelete='cascade')
    ...
    line_qty_ids = fields.One2many('quotation.request.form.line.quantity.new', 'qrf_line_id', 'Line Qty')
    amount_tax = fields.Monetary(
        string='Taxes', currency_field='currency_id', compute='_compute_amount')
    currency_id = fields.Many2one(
        'res.currency', related='company_id.currency_id', store=True,)
    ...
    def create_specification_detail(self):
        ...
        spec = self.env['master.require.new'].search([('active', '=', True)])
        data = []
        if self.qrf_id.type != '3':
            ...
            action = self.env.ref('master_specifications_new.quotation_request_form_line_action_new').read()[0]
            action['res_id'] = self.id
            return action
        else : 
            action = self.env.ref('master_specifications_new.dqups3_line_action_new').read()[0]
            action['res_id'] = self.id
            return action
...
class QuotationRequestFormLineSpecification(models.Model):
    _name = 'quotation.request.form.line.specification.new'
    # _order = "urutan asc"

    qrf_line_id = fields.Many2one('quotation.request.form.line.new', string='QRF')
    jenis_id = fields.Many2one('master.jenis.new', string='Jenis', related='qrf_line_id.jenis_id')
    require_id = fields.Many2one('master.require.new', string='Category')
    urutan = fields.Integer(string='Urutan', related='require_id.urutan')
    specifications_id = fields.Many2one('specifications.new', string='Items', domain="[('jenis_id', '=',jenis_id)]")
...
class QuotationRequestFormLineQuantity(models.Model):
    _name = 'quotation.request.form.line.quantity.new'
    _order = "urutan asc"

    qrf_line_id = fields.Many2one('quotation.request.form.line.new', string='QRF')
    jenis_id    = fields.Many2one('master.jenis.new', string='Jenis', related='qrf_line_id.jenis_id')
    qty_id      = fields.Many2one('master.qty.new', string='Quantity')
...
class QrfAttachment(models.Model):
    ...
    tooling_qc2 = fields.Selection(
        [("altinex", "Altinex"), ("other", "Other")], string='Current Tools producing qc-pass tablets', default='altinex')
    con_ids = fields.One2many('qrf.attachment.conclusion.new', 'qrf_attachment_id', 'Detail')
    ...
    attchment_inform_consent_id = fields.Many2one('inform.consent.attch.new', string='attachment', default=2)
    ...
    def create_qrf_attch_conclusion(self):
        self.ensure_one()
        temp_con = self.env['qrf.template.con.new'].search([
        ...
        action = self.env.ref('master_specifications_new.qrf_att_con_action_new').read()[0]
        action['res_id'] = self.id
        return action
...
class QrfAttachmentConclusion(models.Model):
    _name = 'qrf.attachment.conclusion.new'

    qrf_attachment_id = fields.Many2one('qrf.attachment.new', string='QRF Attch')
...
class StockPicking(models.Model):
    ...
    dqups_id = fields.Many2one('quotation.request.form.new', string="DQUPS")
    sj_attachment_ids = fields.Binary('Surat Jalan', required=True)
    attachment_name = fields.Char('Name')