odoo.define('request_requisition.QtyAtDateWidget', function (require) {
"use strict";
...
var QtyAtDateWidget = Widget.extend({
    template: 'request_requisition.qtyAtDate',
    ...
    _setPopOver: function () {
        var self = this;
        if (!this.data.request_date) {
            return;
        }
        this.data.delivery_date = this.data.request_date.clone().add(this.getSession().getTZOffset(this.data.request_date), 'minutes').format(time.getLangDateFormat());
        // The grid view need a specific date format that could be different than
        // the user one.
        this.data.delivery_date_grid = this.data.request_date.clone().add(this.getSession().getTZOffset(this.data.request_date), 'minutes').format('YYYY-MM-DD');
        this.data.debug = config.isDebug();
        var $content = $(QWeb.render('request_requisition.QtyDetailPopOver', {
            data: this.data,
        }));
