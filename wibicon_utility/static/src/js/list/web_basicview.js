odoo.define('wibicon_utility.WibiconBasicView', function (require) {
    "use strict";
    
    /**
     * The WibiconBasicView is an abstract class designed to share code between views that
    ...
    var WibiconBasicView = AbstractView.extend({
        ...
        _processField: function (viewType, field, attrs) {
            ...
            if (!_.isEmpty(field.views)) {
                ...
                _.each(field.views, function (innerFieldsView, viewType) {
                    viewType = viewType === 'tree' ? 'wibiconlist' : viewType;
            ...
            attrs.mode = attrs.mode === 'tree' ? 'wibiconlist' : attrs.mode;
                ...
                if (attrs.Widget.prototype.useSubview) {
                    ...
                    if (!mode) {
                        if (attrs.views.list && !attrs.views.kanban) {
                            ...
                            mode = 'wibiconlist,kanban';
                        }
                    }
                    if (mode.indexOf(',') !== -1) {
                        mode = config.device.isMobile ? 'kanban' : 'wibiconlist';
                    }
                    attrs.mode = mode;
