odoo.define("wibicon_utility.ListWibiconRenderer", function (require) {
  "use strict";
  ...
  var ListWibiconRenderer = BasicRenderer.extend({
    className: "o_list_view",
    events: {
      mousedown: "_onMouseDown",
      "click .o_optional_columns_dropdown .dropdown-item":
        "_onToggleOptionalColumn",
      "click .o_optional_columns_dropdown_toggle":
        "_onToggleOptionalColumnDropdown",
      ...
      "click .accordion-toggle": "_onToggle",
    ...
    _getOptionalColumnsStorageKeyParts: function () {
      ...
      return {
        fields: _.map(
          this.state.fieldsInfo[this.viewType],
          function (_, fieldName) {
      ...
      _.each(this.arch.children, function (c) {
        if (c.tag !== "control" && c.tag !== "groupby" && c.tag !== "header") {
          ...
          if (c.tag === "button_group") {
            ...
            reject = c.children.every(
              (child) => columnInvisibleFields[child.attrs.name]
            );
      ...
      return _.map(this.columns, function (column) {
        ...
        if (column.attrs.name in aggregateValues) {
          ...
          var formattedValue = formatFunc(value, field, {
            escape: true,
            digits: column.attrs.digits
              ? JSON.parse(column.attrs.digits)
              : undefined,
          });
          $cell
    ...
    _renderBody: function () {
      ...
      //   _.each($rows, (index,row) => {
      //       console.log(index);
      //       console.log(row);
      //     var $newtr = $(`
      //       <tr class="hide-table-padding">
      //       <div id="o_wibicon_acc_${$(row).attr("href")}" class="collapse in p-3">
      //       <div class="row">
    ...
    _renderEmptyRow: function () {
      ...
      console.log("this._getNumberOfCols()", this._getNumberOfCols());
      return $("<tr>").append($td);
    },
    /**
    ...
    _renderGroupButton: function (list, node) {
      ...
      $button.on(
        "click",
        this._onGroupButtonClicked.bind(this, list.groupData, node)
      );
    ...
    _renderGroupRow: function (group, groupLevel) {
      ...
      var aggregateValues = _.mapObject(
      ...
      if (firstAggregateIndex !== -1) {
        ...
        var lastAggregateIndex = _.findLastIndex(
          this.columns,
          function (column) {
            return (
              column.tag === "field" &&
              _.contains(aggregateKeys, column.attrs.name)
            );
          }
        );
        cells = cells.concat(
    ...
    _renderGroupPager: function (group, target) {
      ...
      const pagerMounting = pager.mount(target).then(() => {
        ...
        pager.el.addEventListener("pager-changed", (ev) =>
          this._onPagerChanged(ev, group)
        );
    ...
    _renderHeader: function () {
      //   console.log("this _renderHeader", this);
      var $tr = $("<tr>").append(
    ...
    _renderHeaderCell: function (node) {
      ...
      console.log("this _renderHeader", name);
      var order = this.state.orderedBy;
      var isNodeSorted = order[0] && order[0].name === name;
      var field = this.state.fields[name];
      console.log("this _renderHeaderCell FIelds", field);
      var $th = $("<th>");
      if (name) {
      ...
      if (node.attrs.widget) {
        ...
        const FieldWidget = this.state.fieldsInfo.wibiconlist[name].Widget;
      ...
      $th
        ...
        .addClass(
          (field.sortable ||
            this.state.fieldsInfo.wibiconlist[name].options.allow_order ||
            false) &&
            "o_column_sortable"
        );
      ...
      if (config.isDebug()) {
        var fieldDescr = {
          ...
          attrs: _.extend(
            {},
            node.attrs,
            this.state.fieldsInfo.wibiconlist[name]
          ),
        };
    ...
    //   var $tr = $("<tr/>", { class: "o_data_row accordion accordion-toggle" })
    //     .attr({
    //       "data-id": record.id,
    ...
    //     .append($cells);
      ...
      // <div id="o_wibicon_acc_${record.id}" class="collapse in p-3">
      //     <div class="row">
      //         <div class="col-2">label</div>
      //         <div class="col-6">value 1</div>
      //     </div>
      ...
      // </div>
    var $tr = $(`
        <tr class="o_data_row accordion accordion-toggle" data-id="${record.id}" data-toggle="collapse" href="${record.id}" data-target="${record.id}">
        </tr>
        <tr>
        <td class="hide-table-padding">
        <div id="${record.id}" class="accordion-body collapse in p-3">
        <div class="row">
            <div class="col-2">label</div>
            <div class="col-6">value 1</div>
        </div>
    ...
    _renderOptionalColumnsDropdown: function () {
      ...
      var dropdownMenuClass =
        direction === "rtl" ? "dropdown-menu-left" : "dropdown-menu-right";
      var $dropdown = $("<div>", {
      ...
      this.optionalColumns.forEach(function (col) {
        var txt =
          (col.attrs.string || self.state.fields[col.attrs.name].string) +
    ...
    async _renderView() {
      ...
      if (tableWrapper) {
        ...
        if (this.selection.length) {
          const $checked_rows = this.$("tr").filter((i, el) =>
            this.selection.includes(el.dataset.id)
          );
          $checked_rows
            .find(".o_list_record_selector input")
            .prop("checked", true);
    ...
    _setDecorationClasses: function ($el, decorations, record) {
      for (const [cssClass, expr] of Object.entries(decorations)) {
        $el.toggleClass(
          cssClass,
          py.PY_isTrue(py.evaluate(expr, record.evalContext))
        );
      }
    },
    /**
    ...
    _updateSelection: function () {
      ...
      var self = this;
      var $inputs = this.$(
        "tbody .o_list_record_selector input:visible:not(:disabled)"
      );
    ...
    _onKeyDown: function (ev) {
      ...
      if ($futureCell) {
        ...
        var isInGroupHeader = $futureCell
          .closest("tr")
          .hasClass("o_group_header");
    ...
    _onRowClicked: function (ev) {
      ...
      if (
        !ev.target.closest(".o_list_record_selector") &&
        !$(ev.target).prop("special_click")
      ) {
    ...
    _onToggleCheckbox: function (ev) {
      const $recordSelector = $(ev.target).find(
    ...
    _onToggleSelection: function (ev) {
      ...
      this.$('tbody .o_list_record_selector input:not(":disabled")').prop(
        "checked",
        checked
      );
      this._updateSelection();
