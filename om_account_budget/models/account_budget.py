...
class CrossoveredBudgetLines(models.Model):
    ...
    practical_amount = fields.Monetary(
        compute='_compute_amount', string='Practical Amount', help="Amount really earned/spent.")
    theoritical_amount = fields.Monetary(
        compute='_compute_theoritical_amount', string='Residual Amount',
        help="Amount you are supposed to have earned/spent at this date.")
    percentage = fields.Float(
        compute='_compute_amount', string='Achievement',
    ...
    # Custom
    reserve_amount = fields.Float(string='Booking Amount', compute='get_reserve_amount', digits=0)
    saldo_reserve_amount = fields.Float(string='Saldo Booking Amount', compute='get_reserve_amount', digits=0)
    purchase_order_line_ids = fields.Many2many(
        comodel_name='purchase.order.line',
        compute='_get_purchase_lines',
        string='Purchase Order Lines')
    additional_amount = fields.Float(string='Additional Amount', digits=0)
    stock_move_order_line = fields.Float(string='Pengurang Dari Stock Move',compute='_compute_return_stock_move')
        ...
        for line in self :
            ...
            query = """
                SELECT 
                    pol.id
                FROM 
                    purchase_order_line pol join purchase_order po on pol.order_id = po.id
                WHERE
                    po.state in ('to approve')
                    AND pol.account_budget_id = %s
                    AND po.date_order BETWEEN %s AND %s 
            """


            param = line.general_budget_id.id,line.date_from,line.date_to
            self.env.cr.execute(query,param)
            
            listingId = []
            for res in self.env.cr.fetchall():                
                listingId.append(res[0])
                

            purchase_lines = self.env['purchase.order.line'].search([('id','in',listingId)])
            line.purchase_order_line_ids = purchase_lines

            tmpAmount = 0
            tmpAmount = sum(x.price_total for x in purchase_lines)
            listId = []
            for dataPurchase in purchase_lines:
                listId.append(dataPurchase.order_id.id)
            listPo = list(set(listId))
            for dataOut in listPo:
                tmpAdjustment = 0
                queryTax = """
                        SELECT 
                            adjustment as nilai
                        ...
                        WHERE
                            purchase_id = %s
                    """%(dataOut)
                self._cr.execute(queryTax)
                result = self._cr.dictfetchall()
                tmpAdjustment = 0
                if result:
                    tmpAdjustment = result[0]['nilai'] or 0
                tmpAmount+=tmpAdjustment
            line.reserve_amount = tmpAmount
            line.saldo_reserve_amount = line.theoritical_amount - tmpAmount
    ...
    def _compute_amount(self):
        for line in self :
            all_budget_amount = line.planned_amount + line.additional_amount
            practical_amount = line.get_practical_amount()
            #theoritical_amount = all_budget_amount - practical_amount
            theoritical_amount = all_budget_amount - practical_amount + line.stock_move_order_line
            percentage = 0

            # if(line.general_budget_id.id==60):
            #     print "All budget : ", all_budget_amount
            #     print "Practical  : ", practical_amount
            #     print "Return ::::: ", line.stock_move_order_line

            if all_budget_amount :
                percentage = practical_amount/all_budget_amount*100

            line.theoritical_amount = theoritical_amount
    ...
    def get_practical_amount(self):
        ...
        # practical_amount = super(CrossoveredBudgetLines, self).get_practical_amount()
        practical_amount = 0
        
        listId = []        
        for line in self.purchase_order_line_ids :

            practical_amount += line.price_total
            listId.append(line.order_id.id)


        # --- PENGAMBILAN DATA PURCHASE_TAX_LINE_IDS
        ...
        for dataOut in listPo:
            ...
            practical_amount+=tmpAdjustment

        
        practical_amount-=self.stock_move_order_line
            
        return practical_amount

    def _compute_return_stock_move(self):
        for rec in self:
            ...
            paramReturn = rec.general_budget_id.id,rec.date_from,rec.date_to
            self.env.cr.execute(queryReturn,paramReturn)
            listingReturnId = []
            for res in self.env.cr.fetchall():
                listingReturnId.append(res[0])

            
            purchase_return_lines = self.env['purchase.order.line'].search([('id','in',listingReturnId)])            
            for line in purchase_return_lines :
                
                move_returned_ids = self.env['stock.move'].search([
                    ('origin_returned_move_id','in',line.move_ids.ids),
                    ('location_dest_id.usage', '=', 'supplier'),
                ])

                if(rec.general_budget_id.id==41):
                    for recRec in move_returned_ids:
                        print(" :::: ", recRec.picking_id)

                if move_returned_ids :
                    if line.product_qty :
                        returned_amount += sum(move.price_unit * move.product_uom_qty for move in move_returned_ids)
                    
            rec.stock_move_order_line = returned_amount
