...
class Main(http.Controller):
    ...
    def order_multipart(self, *kwargs):
        ...
        response_content = request.env['ir.ui.view']._render_template(
            'web_custom.multipart', data)
        return request.make_response(response_content, headers=[('Content-Type', 'text/html')])

    @http.route('/confirm-order-monoblock', type='json', auth='public', website=True, cors="*")
    def prepare_order_monoblock(self, **kwargs):
        try:
            basicSpecification = int(
                kwargs['data']['basicSpecification']) if 'basicSpecification' in kwargs['data'] else False
            materials = int(kwargs['data']['materials']
                            ) if 'materials' in kwargs['data'] else False
            tips = int(kwargs['data']['tips']
                       ) if 'tips' in kwargs['data'] else False
            single_or_multi = int(
                kwargs['data']['single_or_multi']) if 'single_or_multi' in kwargs['data'] else False
            dust_cups = int(kwargs['data']['dust_cups']
                            ) if 'dust_cups' in kwargs['data'] else False
            keyway_config = int(
                kwargs['data']['keyway_config']) if 'keyway_config' in kwargs['data'] else False
            keyway_position = int(
                kwargs['data']['keyway_position']) if 'keyway_position' in kwargs['data'] else False
            head_flats = int(kwargs['data']['head_flats']
                             ) if 'head_flats' in kwargs['data'] else False
            heat_treatments = int(
                kwargs['data']['heat_treatments']) if 'heat_treatments' in kwargs['data'] else False
            surface_treatments = int(
                kwargs['data']['surface_treatments']) if 'surface_treatments' in kwargs['data'] else False
            custom_adjustments = int(
                kwargs['data']['custom_adjustments']) if 'custom_adjustments' in kwargs['data'] else False
            fat_options = int(kwargs['data']['fat_options']
                              ) if 'fat_options' in kwargs['data'] else False
            hobbs = int(kwargs['data']['hobbs']
                        ) if 'hobbs' in kwargs['data'] else False
            drawings = int(kwargs['data']['drawings']
                           ) if 'drawings' in kwargs['data'] else False
            punch = int(kwargs['data']['punch']
                        ) if 'punch' in kwargs['data'] else False
            tip = int(kwargs['data']['tip']
                      ) if 'tip' in kwargs['data'] else False
    ...
    def prepare_order_die(self, **kwargs):
        try:
            basics = int(kwargs['data']['basics']
                         ) if 'basics' in kwargs['data'] else False
            materials = int(kwargs['data']['materials']
                            ) if 'materials' in kwargs['data'] else False
            bores = int(kwargs['data']['bores']
                        ) if 'bores' in kwargs['data'] else False
            single_or_multi = int(
                kwargs['data']['single_or_multi']) if 'single_or_multi' in kwargs['data'] else False
            die_screws = int(kwargs['data']['die_screws']
                             ) if 'die_screws' in kwargs['data'] else False
            optional_tapered_bores = int(
                kwargs['data']['optional_tapered_bores']) if 'optional_tapered_bores' in kwargs['data'] else False
            heat_treatments = int(
                kwargs['data']['heat_treatments']) if 'heat_treatments' in kwargs['data'] else False
            surface_treatments = int(
                kwargs['data']['surface_treatments']) if 'surface_treatments' in kwargs['data'] else False
            ...
            die_setting_aligners = int(
                kwargs['data']['die_setting_aligners']) if 'die_setting_aligners' in kwargs['data'] else False
    ...
    def prepare_order_multipart(self, **kwargs):
        try:
            holder_specification_id = int(
                kwargs['data']['holder_specifications']) if 'holder_specifications' in kwargs['data'] else False
            holder_position_id = int(
                kwargs['data']['holder_positions']) if 'holder_positions' in kwargs['data'] else False
            holder_material_id = int(
                kwargs['data']['holder_materials']) if 'holder_materials' in kwargs['data'] else False
            dust_cup_configuration_id = int(
                kwargs['data']['dust_cup_configurations']) if 'dust_cup_configurations' in kwargs['data'] else False
            keyway_configuration_id = int(
                kwargs['data']['keyway_configurations']) if 'keyway_configurations' in kwargs['data'] else False
            keyway_position_id = int(
                kwargs['data']['keyway_positions']) if 'keyway_positions' in kwargs['data'] else False
            head_flat_extension_id = int(
                kwargs['data']['head_flat_extensions']) if 'head_flat_extensions' in kwargs['data'] else False
            holder_heat_treatment_id = int(
                kwargs['data']['holder_heat_treatments']) if 'holder_heat_treatments' in kwargs['data'] else False
            holder_surface_treatment_id = int(
                kwargs['data']['holder_surface_treatments']) if 'holder_surface_treatments' in kwargs['data'] else False
            tip_shape_id = int(
                kwargs['data']['tip_shapes']) if 'tip_shapes' in kwargs['data'] else False
            tip_position_id = int(
                kwargs['data']['tip_positions']) if 'tip_positions' in kwargs['data'] else False
            tip_material_id = int(
                kwargs['data']['tip_materials']) if 'tip_materials' in kwargs['data'] else False
            tip_heat_treatment_id = int(
                kwargs['data']['tip_heat_treatments']) if 'tip_heat_treatments' in kwargs['data'] else False
            tip_surface_treatment_id = int(
                kwargs['data']['tip_surface_treatments']) if 'tip_surface_treatments' in kwargs['data'] else False
            holder_cap_id = int(
                kwargs['data']['holder_caps']) if 'holder_caps' in kwargs['data'] else False
            holder_cap_bore_id = int(
                kwargs['data']['holder_cap_bores']) if 'holder_cap_bores' in kwargs['data'] else False
            holder_cap_surface_id = int(kwargs['data']['holder_cap_surface_treatments']
                                        ) if 'holder_cap_surface_treatments' in kwargs['data'] else False
            custom_adjustment_id = int(
                kwargs['data']['custom_adjustments']) if 'custom_adjustments' in kwargs['data'] else False
            fat_option_id = int(
                kwargs['data']['fat_options']) if 'fat_options' in kwargs['data'] else False
            hobb_id = int(kwargs['data']['hobbs']
                          ) if 'hobbs' in kwargs['data'] else False
            drawing_id = int(kwargs['data']['drawings']
                             ) if 'drawings' in kwargs['data'] else False
    ...
    def success_subscribe(self, **kwargs):
        ...
        if kwargs.get('user'):
            ...
            response_content = request.env['ir.ui.view']._render_template(
                'web_custom.success_subscribe', data)
            return request.make_response(response_content, headers=[('Content-Type', 'text/html')])
