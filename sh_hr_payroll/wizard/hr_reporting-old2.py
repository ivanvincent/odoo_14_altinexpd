...
class HrReporting(models.TransientModel):
    ...
    #             sum(payslip.total_pengurangan_bruto) as total_pengurangan_bruto
    #         FROM (
    #                 SELECT 
	# 					hpl.code, 
	...
	# 					0 as thp_3,
	# 					0 as total_bpjs_perusahaan,
	# 					hpl.total as total_pengurangan_bruto,
	# 					hp.employee_id as karyawan
    #                 FROM 
    #                 hr_payslip hp left join hr_payslip_line hpl on hpl.slip_id = hp.id
	# 				left join hr_employee he on he.id = hp.employee_id
	# 				left join hr_job hj on hj.id = he.job_id
	# 				left join hr_contract hc on hc.id = he.contract_id
    #                 WHERE hpl.code = 'TOTAL_PENGURANG'
    #                 AND hp.month_selection = '{self.month_selection}'
	# 				AND hj.id in {job_ids}
    #             ) AS payslip 
    ...
    #                 "total_pengurangan_bruto": lot.get('total_pengurangan_bruto', ''),
    #             }))
	...
	def action_generate_excel(self):
		...
		left join hr_job hj on hj.id = he.job_id
		left join hr_contract hc on hc.id = he.contract_id
		where hpl.salary_rule_id in %s
		and hp.month_selection = '%s'
		order by hpl.employee_id,hpl.sequence
        ''' % (rules_tuple,self.month_selection)
         self._cr.execute(query)
    	...
    	# WKS 1
         ...
         worksheet.set_column('AO2:AO2', 20)
    

        # WKS 1

         worksheet.merge_range('A2:AO2', report_name , wbf['merge_format'])
         worksheet.merge_range('A3:AO3', 'PERIODE (' + str(self.month_selection) + ')' , wbf['merge_format_2'])
         ...
         worksheet.write('E%s' % (row), 'GAPOK BPJS Kesehatan', wbf['header'])
         worksheet.write('F%s' % (row), 'GAPOK BPJS TK', wbf['header'])
        # worksheet.write('I%s' % (row), 'Tunjangan Keahlian', wbf['header'])
        # worksheet.write('J%s' % (row), 'Tunjangan Shift 3', wbf['header'])
        # # worksheet.write('K%s' % (row), 'Tunjangan Kes Non BPJS', wbf['header'])
        # worksheet.write('K%s' % (row), 'Bonus Proyek', wbf['header'])
        # worksheet.write('L%s' % (row), 'Bonus Bulanan', wbf['header'])
        # worksheet.write('M%s' % (row), 'Total Tjg selain Tjg PPh', wbf['header'])
        # worksheet.write('N%s' % (row), 'Tunjangan PPh 21', wbf['header'])
        # worksheet.write('O%s' % (row), 'BPJS Kes (Perusahaan)', wbf['header'])
        # worksheet.write('P%s' % (row), 'BPJS JHT (Perusahaan)', wbf['header'])
        # worksheet.write('Q%s' % (row), 'BPJS JKK (Perusahaan)', wbf['header'])
        # worksheet.write('R%s' % (row), 'BPJS JP (Perusahaan)', wbf['header'])
        # worksheet.write('S%s' % (row), 'BPJS JKM (Perusahaan)', wbf['header'])
        # worksheet.write('T%s' % (row), 'Total BPJS bayar Perusahaan', wbf['header'])
        # # worksheet.write('U%s' % (row), 'Total BPJS untuk PPh21', wbf['header'])
        # worksheet.write('U%s' % (row), 'Bonus Tahunan', wbf['header'])
        # worksheet.write('V%s' % (row), 'THR', wbf['header'])
        # worksheet.write('W%s' % (row), 'Pesangon (penambah bruto)', wbf['header'])
        # worksheet.write('X%s' % (row), 'Total Komponen Penambah / Bonus Tidak Tetap', wbf['header'])
        # worksheet.write('Y%s' % (row), 'Potongan Resmi lainnya (mengurangi bruto)', wbf['header'])
        # worksheet.write('Z%s' % (row), 'Penghasilan Bruto', wbf['header'])
        # worksheet.write('AA%s' % (row), 'BPJS Kes (Karyawan)', wbf['header'])
        # worksheet.write('AB%s' % (row), 'BPJS JHT (Karyawan)', wbf['header'])
        # worksheet.write('AC%s' % (row), 'BPJS JP (Karyawan)', wbf['header'])
        # worksheet.write('AD%s' % (row), 'Total BPJS bayar Karyawan', wbf['header'])
        # worksheet.write('AE%s' % (row), 'Total pot BPJS u/ PPh21', wbf['header'])
        # # worksheet.write('AE%s' % (row), 'Biaya Jabatan', wbf['header'])
        # worksheet.write('AF%s' % (row), 'Piutang Karyawan (tidak mengurangi bruto)', wbf['header'])
        # worksheet.write('AG%s' % (row), 'Penghasilan Netto', wbf['header'])
        # worksheet.write('AH%s' % (row), 'Penghasilan Netto disetahunkan', wbf['header'])
        # worksheet.write('AI%s' % (row), 'PTKP', wbf['header'])
        # worksheet.write('AJ%s' % (row), 'PKP', wbf['header'])
        # worksheet.write('AK%s' % (row), 'PKP Pembulatan', wbf['header'])
        # worksheet.write('AL%s' % (row), 'PPh 21 Terutang', wbf['header'])
        # worksheet.write('AM%s' % (row), 'PPh 21 Dicicil', wbf['header'])
        # worksheet.write('AN%s' % (row), 'Bayar di Muka', wbf['header'])
        # worksheet.write('AO%s' % (row), 'THP', wbf['header'])
         row += 1
         no = 1 
        # tot_thp = 0
        ...
        # sum_tot_pengurangan_bruto = 0

         for index, rec in enumerate(rslt):
            worksheet.write('A%s' % (row), no, wbf['content_center'])
            if (index+1 < len(rslt) and index - 1 >=0):
                prev_rec = rslt[index-1]
                curr_rec = rec
                if(prev_rec['employee_id'] != curr_rec['employee_id']):
