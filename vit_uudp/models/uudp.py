...
class uudp(models.Model):
    ...
    order_id         = fields.Many2one('purchase.order', string='Purchase Order',domain=[('state', '=', 'purchase'),('invoice_state', '=', 'no')])
    employee_id      = fields.Many2one('hr.employee', string='Pengaju')
    ...
    def button_confirm(self):
        if self.sisa_limit < 1 or self.sisa_limit < sum(self.uudp_ids.mapped('total')):
        ...
        if self.type == 'penyelesaian':
            ...
            if self.uudp_ids:
                ...
                for ajuan in self.uudp_ids:
                    ########################################################################
                    ### 
                    ### comment dulu jika coa debit kosong
                    ### 
                    ########################################################################
                    if not ajuan.coa_debit:
                        raise UserError(_('Account atas %s belum di set!')%(ajuan.description))
                    # if ajuan.uudp_id.employee_id:fff
                    tag_id = False
                    ########################################################################
                    ### 
                    ### comment dulu karena account_analytic_tag_id belum ada di store_id
                    ### 
                    ########################################################################
                    # if ajuan.store_id :
                    ...
                    #account debit
                     ########################################################################
                    ### 
                    ### comment dulu karena account_analytic_tag_id belum ada di department_id
                    ### 
                    ########################################################################
                ...
                if self.ajuan_id:
                    ...
                    _logger = logging.getLogger(__name__)
                    _logger.warning('='*40)
                    _logger.warning(_('Credit account %s belum di set!')%(self.ajuan_id.name))
                    _logger.warning('='*40)
                    ########################################################################
                    ### 
                    ### comment dulu jika coa debit kosong
                    ### 
                    ########################################################################
                    if not self.ajuan_id.coa_debit :
                        raise UserError(_('Credit account %s belum di set!')%(self.ajuan_id.name))
                elif not self.ajuan_id:
                    raise UserError(_('Ajuan dari penyelesaian %s belum di set!')%(self.name))
                
                ########################################################################
                ### 
                ### comment dulu analytic_account_id
                ### 
                ########################################################################
    ...
    def button_confirm_finance(self):
        ...
        #                 s.write({'coa_debit': s.product_id.property_account_expense_id.id})
        #             elif s.product_id.categ_id.property_account_creditor_price_difference_categ:
        #                 s.write({'coa_debit': s.product_id.categ_id.property_account_creditor_price_difference_categ.id})
        #             else:
        ...
        #         for ajuan in result:
        #             #account debit
        #             account_move_line.append((0, 0 ,{'account_id' : ajuan['coa_debit'], 'partner_id': partner, 'name': ajuan['description'], 'operating_unit_id' : False, 'debit' : ajuan['total'], 'date_maturity':self.date})) #, 'analytic_account_id':self.department_id.analytic_account_id.id

        #             total_debit += ajuan['total']          

        #         if self.type == 'penyelesaian':
        #             if total_debit < self.total_ajuan and not self.difference:
        #                 raise AccessError(_('Nilai penyelesaian kurang dari nilai ajuan \n Silahkan isi Difference Account untuk memasukan selisih ke journal entry'))
        #             elif total_debit < self.total_ajuan and self.difference:
        #                 selisih = self.total_ajuan - total_debit
        #                 account_move_line.append((0, 0 ,{'account_id' : self.difference.id, 'partner_id': partner, 'operating_unit_id' : False, 'name' : self.difference.name, 'debit' : selisih, 'date_maturity':self.date})) #, 'analytic_account_id':self.department_id.analytic_account_id.id

        #         #account credit
        #         account_move_line.append((0, 0 ,{'account_id' : self.coa_kredit.id, 'partner_id': partner, 'operating_unit_id' : False, 'name' : self.coa_kredit.name, 'credit' : self.total_ajuan, 'date_maturity':self.date})) #, 'analytic_account_id':self.department_id.analytic_account_id.id

        #         #create journal entry
        #         data={"journal_id":self.journal_id.id,
        #               "ref":self.name + " ("+ self.notes +")",
        #               "date":self.date,
        #               "company_id":self.company_id.id,
        #               "line_ids":account_move_line,}
